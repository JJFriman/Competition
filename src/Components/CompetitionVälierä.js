//CompetitionVälierä.js is for displaying semifinal
...
import VäliEräLopetus from './VäliEräLopetus';
import NewAjastin from './NewAjastin';
function CompetitionVälierä() {
  ...
  const handleDataFetched = (data) => {
    const semifinalistTeams = data.filter((team) => team.semifinalist);
    const sortedTeams = [...semifinalistTeams].sort((a, b) => b.semifinalist - a.semifinalist);
  ...
  const divideTeamsIntoGroups = (teams) => {
    ...
    for (let i = 0; i < teams.length; i += 5) {
      grouped.push(teams.slice(i, i + 5));
    }
  ...
  const handleGroupSelection = (selectedGroupIndex) => {
    ...
    if (socket && socket.connected) {
      const selectedGroupData = {
        groupInfo: `Välierä ${selectedGroupIndex + 1}`,
  ...
  return (
    <div>
      ...
      <VäliEräLopetus eraName={'Välierä'} />
      <CompetitionFetch onDataFetched={handleDataFetched} />
    </div>
  );
}

export default CompetitionVälierä;
